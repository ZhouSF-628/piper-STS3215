...
class PiperFollower(Robot):
    """
    Piper 6-DOF arm + gripper follower robot using Feetech STS3215 motors.

    Joint mapping (ID 1–7):
        shoulder_pan  (ID 1)  — base rotation
        shoulder_lift (ID 2)  — shoulder up/down
        elbow_flex    (ID 3)  — elbow bend
        forearm_roll  (ID 4)  — forearm rotation (between elbow and wrist)
        wrist_flex    (ID 5)  — wrist pitch
        wrist_roll    (ID 6)  — wrist rotation (full-turn for calibration)
        gripper       (ID 7)  — open/close (0–100 %)
    """
    ...
    def configure(self) -> None:
        with self.bus.torque_disabled():
            ...
            for motor in self.bus.motors:
                ...
                if motor == "gripper":
                    self.bus.write("Max_Torque_Limit", motor, 500)  # 50% of max torque
